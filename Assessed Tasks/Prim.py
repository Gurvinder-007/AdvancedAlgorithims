...
Input: Graph with different weights
Output: Weights for each edge of the graph
'''
    ...
    def primMST(self): 
        ...
        for vertex in range(self.V):                            #go through all vertices
            ...
            for v in range(self.V):                             #go through all vertices
                ...
                if self.graph[u][v] > 0 and mstSet[v]==False and key[v] > self.graph[u][v]:#if edge from u to connected node v is > 0 (if there is an edge)
